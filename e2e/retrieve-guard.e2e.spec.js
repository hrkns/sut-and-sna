...
test("retrieve guard dialog appears without breaking subsequent interactions", async ({
  ...
  const cuProSection = await ensureSectionOpen(page, /Cuenta de Producci/i, 1);
  await cuProSection
  ...
  expect(shownDialogs[0]).toMatch(
    /No hay valores del COU disponibles para recuperar/i
  );
